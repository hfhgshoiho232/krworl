    ...
    <header className="bg-white shadow-md">
      <div className="bg-blue-900 text-white py-2 px-4">
        <div className="container mx-auto flex justify-between items-center text-sm">
          <p className="hidden md:block">Contact ICC to discuss your controls support requirements</p>
          <div className="flex items-center gap-4">
            <a href="mailto:sales@iccfzco.com" className="hover:text-yellow-400 transition-colors">sales@iccfzco.com</a>
            <a href="tel:+971404320345" className="hover:text-yellow-400 transition-colors">+971 (0) 4 320 3457</a>
          </div>
        </div>
      </div>
      <div className="container mx-auto py-4 px-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {['Home', 'About', 'Services', 'E-Waste', 'Inventory', 'Contact'].map((item) => (
              <Link key={item} href={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className="text-gray-700 hover:text-blue-600 transition-colors">
                {item}
              </Link>
            ))}
          </nav>
          <div className="flex items-center gap-4">
              ...
              {showResults && searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg border shadow-lg z-50 max-h-[400px] overflow-y-auto divide-y divide-gray-100">
                  {searchResults.map((result, index) => (
                    <Link
                      key={index}
                      href={`/inventory/${result.manufacturer.toLowerCase()}/${result.partNumber.toLowerCase().replace(/\s+/g, '-')}`}
            ...
            <Link href="/request-quote">
              <Button size="lg" className="bg-yellow-500 hover:bg-yellow-600 text-blue-900 font-semibold hidden md:inline-flex">
                REQUEST A QUOTE
              </Button>
            </Link>
            <button
